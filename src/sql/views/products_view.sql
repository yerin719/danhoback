...
CREATE INDEX IF NOT EXISTS idx_product_variants_package_type 
ON product_variants (package_type);
...
SELECT DISTINCT
  unnest(ARRAY[
    ...
    -- Package types
    jsonb_build_object('type', 'package_type', 'value', pv.package_type::text)
  ]) as filter_option
FROM products p
LEFT JOIN product_variants pv ON p.id = pv.product_id
WHERE 
  ...
  AND pv.package_type IS NOT NULL;
